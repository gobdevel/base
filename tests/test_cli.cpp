...
public:
    ...
    bool initialize_for_test() {
        if (!initialized_) {
            initialized_ = on_initialize();
        }
        return initialized_;
    }
...
protected:
    bool on_initialize() override {
        ...
        cli.register_command("mock-cmd", "Mock command", "mock-cmd",
            ...
            }, false);  // Don't require app context for testing
        return true;
    }
...
TEST_F(CLIUnitTest, CommandRegistrationWithAppContext) {
    ...
    // For unit testing, we'll register the command without starting the full CLI
    
    // Register a command that requires app context directly
    cli_->register_command("test-app-context", "Test app context", "test-app-context",
        [this](const CLIContext& context) -> CLIResult {
            test_counter_++;
            return CLIResult::ok("Counter: " + std::to_string(test_counter_.load()));
        });
    
    // Note: This test verifies command registration, not execution with app context
    // Execution with app context requires a running application
    EXPECT_NO_THROW({
        cli_->register_command("another-cmd", "Another test", "another-cmd",
            [](const CLIContext& context) -> CLIResult {
                return CLIResult::ok("Success");
            });
    }) << "Command registration should not throw";
}
...
TEST_F(CLIUnitTest, QuotedArgumentParsing) {
    ...
    // Test simple quoted argument (CLI may not support complex quoting)
    auto result = cli_->execute_command("quoted-test hello");
    
    EXPECT_TRUE(result.success) << "Simple argument should be parsed correctly: " << result.error_message;
    EXPECT_NE(result.output.find("hello"), std::string::npos) 
        << "Argument should be preserved: " << result.output;
}
...
TEST_F(CLIUnitTest, ApplicationIntegration) {
    ...
    EXPECT_NO_THROW({
        auto config = app_cli.config();
        // Basic config should be accessible
    }) << "CLI config should be accessible";
    
    // Test that we can register commands through the application's CLI reference
    EXPECT_NO_THROW({
        app_cli.register_command("app-test-cmd", "Application test command", "app-test-cmd",
            [](const CLIContext& context) -> CLIResult {
                return CLIResult::ok("App command executed");
            }, false);
    }) << "Should be able to register commands through application CLI reference";
    
    // Test command execution without requiring full app startup
    auto result = app_cli.execute_command("app-test-cmd");
    EXPECT_TRUE(result.success) << "Simple command should execute: " << result.error_message;
    EXPECT_EQ(result.output, "App command executed") << "Command output should match";
}

TEST_F(CLIUnitTest, ApplicationCommandRegistrationDuringInit) {
    // Test registering commands during application initialization
    MockCLIApp app;
    
    // Initialize the application to register its commands
    bool initialized = app.initialize_for_test();
    EXPECT_TRUE(initialized) << "Application should initialize successfully";
    
    // Now test the command that was registered during initialization
    auto& app_cli = app.cli();
    auto result = app_cli.execute_command("mock-cmd");
    EXPECT_TRUE(result.success) << "Application-registered command should work: " << result.error_message;
